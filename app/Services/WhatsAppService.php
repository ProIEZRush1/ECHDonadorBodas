...
class WhatsAppService
{
    ...
    public function __construct()
    {
        ...
        if (app()->bound('currentOrganization')) {
            $connection = app('currentOrganization')->whatsappConnections()->where('status', 'connected')->latest('connected_at')->first();
            if ($connection) {
    ...
    public function useConnection(WhatsAppConnection $connection): self
    {
        $this->token = $connection->access_token;
        $this->accountId = $connection->waba_id ?: '';
        $this->phoneNumberId = $connection->phone_number_id;
    ...
    public function getPhoneNumberId(): ?string
    {
        ...
        return Cache::remember('whatsapp_phone_number_id_'.($this->accountId ?: 'default'), 3600, function (): ?string {
